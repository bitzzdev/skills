...
description: Use whenever the user asks to create, build, edit, or debug a Minecraft shaderpack for Iris or OptiFine-compatible shaders. Trigger on phrases like "make a shader", "iris shaderpack", "write a fragment shader for minecraft", "add fog or water or clouds or god rays to my shader", "fix my shaders.properties", or any request to touch vsh, fsh, or glsl files, shaders.properties, or a shaderpacks folder. Also use this skill whenever the user mentions a specific new Minecraft or Iris version, since supported shader features, gbuffers program names, and uniform availability change across versions and memorized knowledge of these may be stale. Enforces looking up current Iris ShaderDoc documentation before writing version-sensitive shader code instead of guessing GLSL uniforms, program names, or shaders.properties keys from memory.
---
